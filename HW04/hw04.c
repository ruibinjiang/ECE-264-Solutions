...
int closestCentroid (int kval, DataPoint * datapoint, Centroid * *centroids)
{
  ...
  long long int diff = distance(datapoint, centroids[mindex]);
  ...
  for (int i = 0; i < kval; i++){
    if (distance(datapoint, centroids[i]) < diff){
      diff = distance(datapoint, centroids[i]);
      mindex = i;
    }
  }
  return mindex;
...
void kmean (int kval, int nval, DataPoint * *datapoints, Centroid * *centroids)
{
	...
	for (int a = 0; a < nval; a++){
		...
		Centroid_addPoint(centroids[a], datapoint[cent]);
	...
	while(unfinished){
		...
		for (c = 0; c < nval; c++){
			Centroid_addPoint(centroids[c], datapoint[centroids[c]->cluster]);
		}
